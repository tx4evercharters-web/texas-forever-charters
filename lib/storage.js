...
async function importHistoricalBookings(rows) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (!email) {
        results.errors.push({ row: i + 1, error: 'Missing customer_email' });
        continue;
      }
      ...
      // Check before upsert so we know whether the customer was new or matched
      const existed = !!(await findCustomerByEmail(email));
      ...
      if (existed) results.customers_matched += 1;
      else         results.customers_created += 1;
