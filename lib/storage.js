...
async function addManualBooking(booking) {
  ...
  /* Hoist portal_token computation so we can return it to callers.
     handleAddBooking needs it to thread through to sendConfirmationEmails
     for the portal-link section without an extra Supabase round-trip. */
  const portal_token = booking.portal_token || generatePortalToken();
  ...
  await request('POST', '/bookings', record, { Prefer: 'return=minimal' });
  return { session_id, customer_id, portal_token };
}
...
/* Hard-delete a single lead row by id. Powers the admin Leads tab's
   delete action. Idempotent: deleting an already-deleted row returns
   true (PostgREST returns 204 with no rows affected). Mirrors the
   shape of deleteBookingRow + deleteCustomer. */
...
module.exports = {
  ...
  patchLead, listLeads, listRecentLeads, deleteStaleLeads, deleteLead,
