...
async function saveBooking(booking) {
  ...
  const taxAmount         = Math.round((subtotalPreFees + adminFee) * 0.085 * 100) / 100;
  const processingFee     = Math.round((subtotalPreFees + adminFee + taxAmount) * 0.029 * 100) / 100;
  const charterSubtotal   = parseFloat((subtotalPreFees - add_on_total).toFixed(2));
  const tax_amount        = parseFloat(taxAmount.toFixed(2));
  const charter_subtotal  = parseFloat(charterSubtotal.toFixed(2));
