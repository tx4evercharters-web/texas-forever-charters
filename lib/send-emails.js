...
// Liability waiver link sent in the confirmation email — REPLACE WITH ACTUAL WAIVERSIGN URL.
// Until then, swap the env var WAIVER_URL on Vercel to override without a code change.
const WAIVER_URL = process.env.WAIVER_URL || 'https://www.texasforevercharters.com/terms.html#liability';
...
function buildCustomerEmail(d) {
  ...
  // Liability waiver — required for all guests before boarding
  html += sectionBox('Liability Waiver — Required',
    '<div style="font-size:14px;color:#374151;line-height:1.7;">' +
      '<strong>All guests must sign the liability waiver before boarding.</strong> ' +
      'Share this link with your group:' +
      '<div style="text-align:center;margin:16px 0 6px;">' +
        '<a href="' + WAIVER_URL + '" style="display:inline-block;background:#1B2A6B;color:#FFFFFF;' +
          'font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;' +
          'text-decoration:none;padding:12px 24px;border-radius:6px;">Sign the Waiver</a>' +
      '</div>' +
      '<div style="font-size:12px;color:#6B7280;text-align:center;">' + WAIVER_URL + '</div>' +
    '</div>');
  html += divider();
...
module.exports = { sendConfirmationEmails, postToResend, sendCancellationEmail, sendRefundEmail, sendDamageChargeEmail };
