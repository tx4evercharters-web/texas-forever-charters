...
/* Waiver-status section for T-3 and T-1 charter reminders. Three states
   (none / partial / all) × two urgency tones (soft for T-3, sharp for T-1).
   Color treatment is shared per state — red box for "none", amber for
   "partial", green for "all" — while copy varies per tone. waiverStatus is
   { signed_count, party_size, status } where status is 'none' | 'partial'
   | 'all'. The section sits inside a white-background sectionBox wrapper
   so it visually integrates with the surrounding email sections. */
function waiverStatusSectionHtml(waiverStatus, isUrgent) {
  const signed_count = Number(waiverStatus.signed_count) || 0;
  const party_size   = Number(waiverStatus.party_size)   || 0;
  const status       = waiverStatus.status;

  let bgColor, borderColor, textColor, headlineColor, headline, body;
  ...
  if (status === 'all') {
    ...
    headline      = '&#10003; Waiver Complete';
    body          = 'All ' + party_size + ' guests in your party have signed. You\'re cleared to board.';
  } else if (status === 'partial') {
    const remaining = Math.max(0, party_size - signed_count);
    bgColor       = '#FFFBEB';
    borderColor   = '#FDE68A';
    textColor     = '#92400E';
    headlineColor = '#B45309';
    headline      = 'Waiver: ' + signed_count + ' of ' + party_size + ' Have Signed';
    body          = isUrgent
      ? remaining + ' guest' + (remaining === 1 ? '' : 's') + ' must sign before tomorrow. Share your booking portal with anyone who has not signed yet.'
      : 'The remaining ' + remaining + ' guest' + (remaining === 1 ? '' : 's') + ' need to sign before charter day. Share your booking portal with anyone who has not signed yet.';
  } else {
    bgColor       = '#FEF2F2';
    borderColor   = '#FCA5A5';
    textColor     = '#991B1B';
    headlineColor = '#B91C1C';
    headline      = isUrgent ? 'Urgent: Waiver Not Signed' : 'Waiver: Not Signed Yet';
    body          = isUrgent
      ? 'No one in your party has signed the liability waiver. Every guest must sign before boarding tomorrow. Share your booking portal with your group now.'
      : 'No one in your party has signed the liability waiver yet. Every guest must sign before boarding. Share your booking portal with your group so they can sign before charter day.';
  }

  return '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>' +
    '<td style="background:#FFFFFF;padding:24px 32px;border-left:1px solid #E5E7EB;border-right:1px solid #E5E7EB;">' +
    '<div style="background:' + bgColor + ';border:1px solid ' + borderColor + ';border-radius:8px;padding:16px 18px;">' +
      '<div style="font-size:14px;font-weight:700;color:' + headlineColor + ';margin-bottom:6px;">' + headline + '</div>' +
      '<div style="font-size:13px;color:' + textColor + ';line-height:1.6;">' + body + '</div>' +
    '</div>' +
    '</td></tr></table>';
}

/* T-3 charter reminder. Fires from api/cron-charter-reminders.js when a
   booking's date is exactly 3 days from current Central. Soft urgency tone.
   Includes waiver status, portal CTA, conditional balance reminder, and
   standard booking-details + call-CTA scaffolding. */
function buildT3ReminderEmail(b, waiverStatus) {
  let html = emailHeader();
  ...
  html += '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>' +
    ...
    '<div style="font-size:28px;">&#9975;</div>' +
    '<div style="font-size:24px;font-weight:900;color:#FFFFFF;letter-spacing:2px;' +
      'text-transform:uppercase;margin:10px 0 8px;font-family:Arial,sans-serif;">3 Days to Go</div>' +
    '<div style="font-size:14px;color:rgba(255,255,255,0.7);line-height:1.7;">' +
      'Your charter is coming up. Let\'s make sure you\'re set.' +
    '</div></td></tr></table>';

  // Message
  html += sectionBox('Message',
    '<div style="font-size:14px;color:#374151;line-height:1.7;">' +
      'Hi ' + firstNameOf(b) + ', your charter is in <strong>3 days</strong>. Here\'s what you need to know to be ready.' +
    '</div>');
  html += divider();

  // Waiver status section (soft tone)
  html += waiverStatusSectionHtml(waiverStatus, false);
  html += divider();

  // Portal CTA (Pattern A — primary action)
  const portalUrl = portalUrlFor(b);
  if (portalUrl) {
    html += sectionBox('Your Booking Portal',
      '<div style="font-size:14px;color:#374151;line-height:1.7;">' +
        'Your booking portal has directions to the dock, the full list of what to bring, the rules, FAQ, and your waiver status. Check it now so you\'re ready on charter day.' +
        '<div style="text-align:center;margin:16px 0 6px;">' +
          '<a href="' + portalUrl + '" style="display:inline-block;background:#1B2A6B;color:#FFFFFF;' +
            'font-size:13px;font-weight:700;letter-spacing:2px;text-transform:uppercase;' +
            'text-decoration:none;padding:13px 28px;border-radius:6px;">View Your Booking Portal</a>' +
  ...
  if (b.paid_in_full === false && parseFloat(b.remaining_balance || 0) > 0) {
    html += sectionBox('Balance Outstanding',
      '<div style="font-size:14px;color:#374151;line-height:1.7;">' +
        'Your balance of <strong>' + formatMoneyDollars(b.remaining_balance) + '</strong> is still outstanding. ' +
        (portalUrl
          ? 'You can pay it from <a href="' + portalUrl + '" style="color:#1B2A6B;text-decoration:none;font-weight:700;">your booking portal</a>.'
          : 'Call or text <a href="tel:+17373681669" style="color:#C8102E;text-decoration:none;font-weight:700;">(737) 368-1669</a> to pay.') +
      '</div>');
    html += divider();
  }
...
/* T-1 charter reminder. Fires from api/cron-charter-reminders.js when a
   booking's date is exactly 1 day from current Central. Sharp urgency tone
   for waiver status. Adds a day-of logistics section ahead of the portal
   CTA. */
function buildT1ReminderEmail(b, waiverStatus) {
  let html = emailHeader();
  ...
  html += '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>' +
    ...
    '<div style="font-size:28px;">&#127749;</div>' +
    '<div style="font-size:24px;font-weight:900;color:#FFFFFF;letter-spacing:2px;' +
      'text-transform:uppercase;margin:10px 0 8px;font-family:Arial,sans-serif;">Tomorrow\'s the Day</div>' +
    '<div style="font-size:14px;color:rgba(255,255,255,0.7);line-height:1.7;">' +
  ...
  html += sectionBox('Message',
    '<div style="font-size:14px;color:#374151;line-height:1.7;">' +
      'Hi ' + firstNameOf(b) + ', your charter is <strong>tomorrow</strong>. Here\'s what you need to know for day-of.' +
    '</div>');
  html += divider();

  // Day-of logistics
  html += sectionBox('Where to Go',
    '<div style="font-size:14px;color:#374151;line-height:1.7;">' +
      '<strong>Volente Beach Waterpark &amp; Resort</strong><br>' +
      '16107 FM 2769, Leander, TX 78641<br><br>' +
      'Plan to be at the dock <strong>15 minutes before your departure time</strong>. Your captain runs a 10-minute safety briefing before you push off, covering the boat layout, the rules, and where everything is. The earlier you\'re aboard, the more lake time you get.' +
    '</div>');
  html += divider();

  // Waiver status section (sharp tone)
  html += waiverStatusSectionHtml(waiverStatus, true);
  html += divider();

  // Portal CTA (Pattern A)
  const portalUrl = portalUrlFor(b);
  if (portalUrl) {
    html += sectionBox('Open Your Portal',
      '<div style="font-size:14px;color:#374151;line-height:1.7;">' +
        'Open your portal one more time to verify the address, confirm what to bring, and check directions to the dock.' +
        '<div style="text-align:center;margin:16px 0 6px;">' +
          '<a href="' + portalUrl + '" style="display:inline-block;background:#1B2A6B;color:#FFFFFF;' +
            'font-size:13px;font-weight:700;letter-spacing:2px;text-transform:uppercase;' +
            'text-decoration:none;padding:13px 28px;border-radius:6px;">Open Your Portal</a>' +
        '</div>' +
        '<div style="font-size:12px;color:#6B7280;text-align:center;word-break:break-all;">' + portalUrl + '</div>' +
      '</div>');
    html += divider();
  ...
  // Conditional balance reminder (sharper tone for day-before)
  if (b.paid_in_full === false && parseFloat(b.remaining_balance || 0) > 0) {
    html += sectionBox('Balance Outstanding',
      '<div style="font-size:14px;color:#374151;line-height:1.7;">' +
        '<strong>Your balance of ' + formatMoneyDollars(b.remaining_balance) + ' is still outstanding.</strong> ' +
        (portalUrl
          ? 'You can pay it from <a href="' + portalUrl + '" style="color:#1B2A6B;text-decoration:none;font-weight:700;">your booking portal</a>.'
          : 'Call or text <a href="tel:+17373681669" style="color:#C8102E;text-decoration:none;font-weight:700;">(737) 368-1669</a> to pay.') +
      '</div>');
  ...
  // Call CTA (day-of phrasing)
  html += callCtaBlock('Day-of Questions? Call (737) 368-1669');
  html += divider();
  html += emailFooter();
...
async function sendT3ReminderEmail(booking, waiverStatus) {
  if (!booking || !booking.customer_email) {
    console.log('[send-emails] sendT3ReminderEmail: no customer_email — skipping');
    return null;
  }
  if (!process.env.RESEND_API_KEY) throw new Error('RESEND_API_KEY not configured');
  console.log('[send-emails] Sending T-3 charter reminder to:', booking.customer_email, 'session:', booking.session_id, '| waiver:', waiverStatus && waiverStatus.status);
  const result = await postToResend({
    from:     FROM_EMAIL,
    to:       [booking.customer_email],
    reply_to: BUSINESS_EMAIL,
    subject:  'Your charter is in 3 days — here\'s what to know',
    html:     buildT3ReminderEmail(booking, waiverStatus),
  });
  console.log('[send-emails] T-3 reminder sent OK. Resend id:', result.id);
  return result;
}

async function sendT1ReminderEmail(booking, waiverStatus) {
  if (!booking || !booking.customer_email) {
    console.log('[send-emails] sendT1ReminderEmail: no customer_email — skipping');
    return null;
  }
  if (!process.env.RESEND_API_KEY) throw new Error('RESEND_API_KEY not configured');
  console.log('[send-emails] Sending T-1 charter reminder to:', booking.customer_email, 'session:', booking.session_id, '| waiver:', waiverStatus && waiverStatus.status);
  const result = await postToResend({
    from:     FROM_EMAIL,
    to:       [booking.customer_email],
    reply_to: BUSINESS_EMAIL,
    subject:  'Tomorrow\'s the day — your charter checklist',
    html:     buildT1ReminderEmail(booking, waiverStatus),
  });
  console.log('[send-emails] T-1 reminder sent OK. Resend id:', result.id);
  return result;
}

function buildFriendlyReminderEmail(b) {
  const amount = moneyDollars(b.remaining_balance);
  const portalUrl = portalUrlFor(b);
  const portalBlock = portalUrl
    ? '<div style="margin-top:14px;"><strong>To pay your balance, <a href="' + portalUrl + '" style="color:#1B2A6B;text-decoration:none;">open your booking portal</a>.</strong> You can pay, view directions to the dock, sign your waiver, and see what to bring, all from one page.</div>'
    : '';
  let html = emailHeader();
  html += '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>' +
    '<td style="background:#0A1030;padding:32px;text-align:center;' +
  ...
  html += sectionBox('Message',
    '<div style="font-size:14px;color:#374151;line-height:1.7;">' +
      ...
      'is due in <strong>7 days</strong>. Pay early and you\'re all set for charter day.' +
      portalBlock +
    '</div>');
  html += divider();
  html += bookingDetailsBlock(b);
...
function buildDueTodayEmail(b) {
  const amount = moneyDollars(b.remaining_balance);
  const portalUrl = portalUrlFor(b);
  const portalBlock = portalUrl
    ? '<div style="margin-top:14px;"><strong>To pay, <a href="' + portalUrl + '" style="color:#1B2A6B;text-decoration:none;">open your booking portal</a>.</strong> You can also call (737) 368-1669 if you prefer to pay by phone.</div>'
    : '';
  let html = emailHeader();
  html += '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>' +
    '<td style="background:#0A1030;padding:32px;text-align:center;' +
...
function buildFinalNoticeEmail(b) {
  const amount = moneyDollars(b.remaining_balance);
  const portalUrl = portalUrlFor(b);
  const portalBlock = portalUrl
    ? '<div style="margin-top:14px;"><strong>You can also <a href="' + portalUrl + '" style="color:#1B2A6B;text-decoration:none;">pay from your booking portal</a> right now.</strong></div>'
    : '';
  let html = emailHeader();
  html += '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>' +
...
async function sendFriendlyReminderEmail(booking) {
  if (!booking || !booking.customer_email) {
  ...
  const result = await postToResend({
    ...
    html:     buildFriendlyReminderEmail(booking),
  });
  console.log('[send-emails] Friendly reminder sent OK. Resend id:', result.id);
  return result;
}

async function sendDueTodayEmail(booking) {
  if (!booking || !booking.customer_email) {
  ...
  const result = await postToResend({
    ...
    html:     buildDueTodayEmail(booking),
  });
...
async function sendFinalNoticeEmail(booking) {
  if (!booking || !booking.customer_email) {
  ...
  const result = await postToResend({
    ...
    html:     buildFinalNoticeEmail(booking),
  });
...
module.exports = {
  ...
  sendT3ReminderEmail, sendT1ReminderEmail,
